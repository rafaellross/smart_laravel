	...
	public function add(TimeSheet $timeSheet){
	    ...
	    foreach ($timeSheet->days()->get() as $day) {
	    	...
	    	foreach ($day->dayJobs()->get() as $job) {
	    		...
	    		//Write job code
	    		$this->Cell($width_days,5, $job->job["code"] .  (is_null($job->job["code"]) ? "" : "/") . Hour::convertToDecimal($job->hours()) . " " . ($job["tafe"] ? "(Tafe) " : "") . ($job["sick"] ? "(Sick) " : "") . ($job["public_holiday"] ? "(P. Holiday) " : "") , 1,2,'C', false);
	    	}
